...
function handleText(client, text){
  ...
  if(msg.type === "race" && typeof msg.text === "string"){
    ...
    client.state = normalizeState(msg.state);
    broadcast(client.room, {
      type: "state",
      from: client.id,
      name: client.name,
    ...
    client.state = Object.assign(normalizeState(msg.state), { finished: true });
    broadcast(client.room, finishForRoom(client.room, msg.reason || "finished"));
  }
}
...
function finishForRoom(room, reason){
  const players = playersFor(room).map(player => ({
    id: player.id,
    name: player.name,
    state: player.state || normalizeState({})
  }));
...
function comparePlayers(a, b){
  ...
  const progressDelta = (bState.progress || 0) - (aState.progress || 0);
  if(progressDelta !== 0) return progressDelta;

  const finishDelta = Number(!!bState.finished) - Number(!!aState.finished);
  if(finishDelta !== 0) return finishDelta;

  const wpmDelta = (bState.wpm || 0) - (aState.wpm || 0);
  if(wpmDelta !== 0) return wpmDelta;

  const accuracyDelta = (bState.accuracy || 0) - (aState.accuracy || 0);
  if(accuracyDelta !== 0) return accuracyDelta;

  return (aState.errors || 0) - (bState.errors || 0);
}
...
function playersFor(room){
  const members = rooms.get(room);
  if(!members) return [];
  return Array.from(members, client => ({
    id: client.id,
    name: client.name,
    state: client.state || normalizeState({})
  }));
}
...
function normalizeState(state){
  const src = state || {};
  const accuracy = Number.isFinite(Number(src.accuracy)) ? Number(src.accuracy) : 100;
  return {
